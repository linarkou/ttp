...
public class NonDominatedSet {
    ...
    public List<TTPSolution> entries = new LinkedList<>();
     ...
     */
    public boolean add(TTPSolution s) {

        boolean isAdded = true;

        if (s.isWorstSolution()) {
            isAdded = false;
            return false;
        }


        for (Iterator<TTPSolution> it = entries.iterator(); it.hasNext(); ) {
            TTPSolution other = it.next();
